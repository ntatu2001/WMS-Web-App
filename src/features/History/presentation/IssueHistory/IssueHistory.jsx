...
import { exportIssueToPdf } from '../../../GoodIssue/utils/exportIssuePdf.js';
...
const IssueHistory = () => {
  ...
  const pdfLineItems = listIssueStorage.map((item) => ({
    id: item.id,
    materialName: item.materialName,
    materialId: item.materialId,
    unitOfMeasure: item.unitOfMeasure,
  ...
  const handleExportPdf = async (formValues) => {
    try {
      await exportIssueToPdf(selectedItem, pdfLineItems, formValues);
      toast.success(t('history.pdfExportSuccess'), { position: 'top-right', autoClose: 3000 });
      setShowPdfModal(false);
    } catch (error) {
      console.error('Export issue PDF error:', error);
